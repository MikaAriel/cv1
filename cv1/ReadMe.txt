================================================================================
    MFC 라이브러리 : cv1 프로젝트 개요
===============================================================================

응용 프로그램 마법사에서 이 cv1 응용 프로그램을 만들었습니다. 이 응용 프로그램은 MFC의 기본 사용법을 보여 줄 뿐만 아니라 응용 프로그램을 작성하기 위한 기본 구조를 제공합니다.

cv1 응용 프로그램을 구성하는 각 파일에 대한
요약 설명이 포함되어 있습니다.
...
cv1.h
    ...
    여기에는 resource.h를 비롯한 다른 프로젝트별 헤더가 포함되어 있으며 Ccv1App 응용 프로그램 클래스가 선언되어 있습니다.

cv1.cpp
    이는 응용 프로그램 클래스 Ccv1App가 포함된 기본 응용 프로그램 소스 파일입니다.
...
res\cv1.ico
    아이콘 파일이며, 응용 프로그램의 아이콘으로 사용됩니다. 이 아이콘은 기본 리소스 파일인 cv1.rc에 의해 포함됩니다.
...
cv1Doc.h, cv1Doc.cpp - 문서
    이 파일에는 Ccv1Doc 클래스가 포함됩니다. 이 파일을 편집하여 특수 문서 데이터를 추가하고 Ccv1Doc::Serialize를 통해 파일 저장 및 로드를 구현합니다.
    문서에는 다음과 같은 문자열이 있습니다.
        파일 확장명:      raw
        파일 형식 ID:        cv1.Document
        주 프레임 캡션:  영상처리
        Doc 형식 이름:       cv1
        필터 이름:         cv1 Files (*.raw)
        파일의 새 약식 이름: cv1
        파일 형식의 긴 이름: cv1.Document

cv1View.h, cv1View.cpp - 문서 뷰
    이 파일에는 Ccv1View 클래스가 포함됩니다.
    Ccv1View 개체는 Ccv1Doc 개체를 보는 데 사용됩니다.

res\cv1Doc.ico
    아이콘 파일이며, Ccv1Doc 클래스 MDII 자식 창의 아이콘으로 사용됩니다. 이 아이콘은 기본 리소스 파일인 cv1.rc에 의해 포함됩니다.
...
StdAfx.h, StdAfx.cpp
    이 파일은 미리 컴파일된 헤더(PCH) 파일 cv1.pch와 미리 컴파일된 형식(PCT) 파일 StdAfx.obj를 빌드하는 데 사용됩니다.
